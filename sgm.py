...
left_costs = np.uint8(255*np.argmin(left_cost_volume, axis=2) / SEARCH_BLOCK_SIZE)

left_disparity_map = np.uint8(255*select_disparity(left_aggregation_volume) / SEARCH_BLOCK_SIZE)
right_disparity_map = np.uint8(255*select_disparity(right_aggregation_volume) / SEARCH_BLOCK_SIZE)
